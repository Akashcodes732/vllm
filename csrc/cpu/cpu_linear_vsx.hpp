...
inline at::Tensor pack_weight(at::Tensor& weight) {
  ...
  TORCH_CHECK(weight.dim() == 2, "vsx_pack_weight: weight must be 2D, got ",
              weight.dim(), "D");
  ...
  TORCH_CHECK(N % GemmT::NSize == 0, "vsx_pack_weight: N (", N,
              ") must be a multiple of ", GemmT::NSize, " (NSize)");
...
inline at::Tensor bf16_mm(const at::Tensor& A, const at::Tensor& packed_B,
  ...
  TORCH_CHECK(packed_B.dim() == 2, "vsx_bf16_mm: packed_B must be 2D, got ",
              packed_B.dim(), "D");
  ...
  if (bias.has_value()) {
    ...
    TORCH_CHECK(bias->numel() == N, "vsx_bf16_mm: bias size (", bias->numel(),
                ") must equal N (", N, ")");
  }
  ...
  // grain_size=0 lets PyTorch auto-distribute work across available threads.
  int64_t total_tiles = static_cast<int64_t>(num_m_tiles) * num_n_tiles;
  at::parallel_for(0, total_tiles, 0, [&](int64_t begin, int64_t end) {
    // Per-thread FP32 accumulation buffer: 8*16*4 = 512 bytes.
    ...
    for (int64_t idx = begin; idx < end; ++idx) {
      ...
      int32_t m_actual =
          static_cast<int32_t>(std::min(static_cast<int64_t>(MaxM), M - m_off));
      ...
      // Single GEMM call with full K — keeps MMA accumulators in registers.
      gemm.gemm(const_cast<at::BFloat16*>(a_ptr + m_off * K),
                const_cast<at::BFloat16*>(b_tile), c_tile_buf, m_actual,
      ...
      if (bias_col) {
        cpu_micro_gemm::bias_epilogue<NTile, at::BFloat16>(
            c_tile_buf, out_tile_row, const_cast<at::BFloat16*>(bias_col),
            m_actual, NTile, static_cast<int32_t>(N));
      } else {
        cpu_micro_gemm::default_epilogue<NTile, at::BFloat16>(
            c_tile_buf, out_tile_row, m_actual, NTile, static_cast<int32_t>(N));
      }
    }
  });
